...
const AccountScreen = ({navigation}) => {
  const { getUserToken, signOut } = React.useContext(AuthContext);
  ...
  React.useEffect(async() => {
    ...
    var userData = await DataFetcher.getUserData(userToken);
  ...
  const handleConfirm = async(pfp, username, firstName, lastName, bio) => {
    var user_id = await getUserToken();
    var newUser = await DataFetcher.editUser(user_id, pfp, username, firstName, lastName, bio);

    setUserData(newUser);
    setModalVisible(false);
    if(newUser.profile_pic != null){
      var image = await DataFetcher.getImage(newUser.profile_pic);
      setPfp(image);
    }
  }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={{alignItems: 'flex-end'}}>
        <TouchableOpacity
            ...
            onPress={signOut}
        >
            <View style={[styles.signOut, {backgroundColor: "#000000"}]}>
      ...
      >  
        <AccountModal 
          userData={userData}
          prevPfp={userData.profile_pic ? pfp : null}
